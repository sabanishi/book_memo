...
- CPUの効率化を求めるためには、連続して使用するデータは連続したメモリに入っている事が必要である
  - しかし、本書で述べられた抽象化などのテクニックを使用すると、メモリの連続化は実現できない
    - インターフェースを使用する事は、あちこちにあるメモリを飛び回る事と同義だから

## 18章 ダーティフラグ

- 何か計算しなければならないが、それを今行う必要がないならば、必要になるまで計算を遅延する
  - 「計算しなければならない」というフラグだけつけておき、必要な時にフラグがついている物を計算する

- セーブ機能などに使えそう
  - Firebaseへのデータアップロードシステムに応用できそう感はある

## 19章 オブジェクトプール

- 使い方
  - あらかじめ必要な分だけオブジェクトを作っておき、それに「使用中か」を判別するフラグを用意する
  - オブジェクトが必要になったらプールから未使用な物を1個取り出して「使用中」にする
  - オブジェクトが不要になったら「未使用」に戻す

- プールが足りなくなった場合
  - オブジェクトを生成しない
    - パーティクルシステムのように、粒1個1個自体に価値が無い場合、プール全てが使われているならばユーザーに充分気付かれない
  - 既存のオブジェクトから1個破壊する
    - サウンドシステムのような場合、既に鳴っている音のうち一番小さいものを削除したら気付かれないかもしれない
  - プールサイズを大きくする
    - 不要になった時にプールサイズをまた元に戻すのかを考える必要がある

## 20章

- 当たり判定を計算する事を考えた時、その計算量は$O(n^2)$である
  - そのため、画面全体の全オブジェクト間で当たり判定を計算するとヤバい

- そこで、空間を格子状に分割して、各格子内のオブジェクト同士で当たり判定を計算する事で、計算量を削減する
